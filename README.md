...
Aethra now behaves as a live-local dashboard first. Its top-level **Refresh local daemon data** action manually reads the supported local daemon metadata surfaces (`/health`, `/v1/status/version`, `/v1/models`, `/v1/models/inventory`, `/v1/models/readiness`, `/v1/status/models`, `/v1/capabilities`, `/v1/operations/summary`, `/v1/routing/policy-summary`, `/v1/evidence/packages`, `/v1/audit/events`, and `/v1/metrics/sustainability?period=30d`). The main dashboard groups those surfaces into core daemon status, models/readiness, routing/operations, evidence/audit, and sustainability; shows a suggested review flow; and names what the dashboard proves and does not do. It shows live local, not loaded, unavailable, or failed state per surface and does not silently replace missing daemon data with fixtures. Fixtures remain available only as clearly labeled offline preview/test/demo data. Aethra has demo smoke and review-readiness tests plus a manual review checklist for screenshot/reviewer passes. The checklist does not generate screenshots. Aethra stays read-only with no startup auto-load, polling, persistence of live responses, telemetry, cloud calls, prompt submission, route execution, connector mutation, policy mutation, package generation, package validation, upload/download/delete actions, or model/runner controls.
