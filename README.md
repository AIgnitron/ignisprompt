...
- Aethra live-local dashboard polish with clearer grouped status cards, endpoint matrix, suggested review flow, and explicit non-goals for demo and reviewer readiness
- Aethra product-surface cleanup that keeps detailed local-preview, status, and safety guidance in Help while main pages stay focused on concise status, data cards, tables, actions, and empty states
- Aethra demo smoke and review-readiness coverage plus a manual review checklist for screenshots/reviewer passes
...
- `/health`, `/v1/models`, `/v1/models/inventory`, `/v1/models/readiness`, `/v1/capabilities`, `/v1/status/models`, `/v1/operations/summary`, `/v1/routing/policy-summary`, `/v1/evidence/packages`, `/v1/route/explain`, `/v1/chat/completions`, `/v1/audit/events`, and `/v1/metrics/sustainability?period=30d` respond locally
...
- [Aethra review checklist](docs/AETHRA_REVIEW_CHECKLIST.md)
...
- `GET /health`
- `GET /v1/models`
- `GET /v1/models/inventory` for read-only local model inventory metadata
- `GET /v1/models/readiness` for read-only local model readiness hints
...
- `GET /v1/evidence/packages` for read-only local evidence package index metadata
...
`doctor` checks the local daemon health, version status, model manifest, connector/capability status, and model and runner status hint endpoints, then reports local next steps for common failures. Its model inventory, model readiness, operations summary, routing policy summary, evidence package index, and sustainability metrics checks are informational. The command is local-only and does not perform telemetry, cloud calls, GitHub calls, update checks, external lookup, persistence, uploads, connector controls, model controls, runner controls, routing controls, package generation, or package validation.
...
`model-inventory` reads `GET /v1/models/inventory` and prints read-only local model inventory metadata from safe local model directories. It reports filenames or safe display paths, formats, sizes, simple filename-derived hints, and summary counts. It does not execute models, read model contents, hash model files, download or delete model files, scan outside configured local model roots, return secrets, call cloud services, or imply model quality, production readiness, compliance, or legal accuracy.

`model-readiness` reads `GET /v1/models/readiness` and prints read-only local model readiness hints derived from manifest declarations, local inventory metadata, file format hints, shard filename hints, and runner status hints. It does not execute models, route requests, download or delete model files, upload files, mutate manifests, read full model contents, hash large files, call cloud services, return secrets, or imply model quality, production readiness, compliance, or legal accuracy.
...
`evidence-packages` reads `GET /v1/evidence/packages` and prints read-only local evidence package index metadata from the repo's local evidence root. It reports safe relative package names, package type guesses, cheap file counts and sizes, artifact indicators, warnings, and boundary notes. It does not generate packages, validate package correctness, upload, download, delete, mutate files, read full evidence contents, expose raw prompts, expose raw request bodies, expose audit event bodies, call cloud services, send telemetry, or imply attestation, certification, compliance, legal correctness, or production readiness.

Aethra now behaves as a live-local dashboard first. Its top-level **Refresh local daemon data** action manually reads the supported local daemon metadata surfaces (`/health`, `/v1/status/version`, `/v1/models`, `/v1/models/inventory`, `/v1/models/readiness`, `/v1/status/models`, `/v1/capabilities`, `/v1/operations/summary`, `/v1/routing/policy-summary`, `/v1/evidence/packages`, `/v1/audit/events`, and `/v1/metrics/sustainability?period=30d`). Overview keeps the full live-local dashboard, grouped endpoint state, endpoint matrix, and suggested review flow. Detail pages focus on their own product content and no longer repeat large explanatory/status guidance blocks; the Help page carries detailed local-preview, data source, troubleshooting, and safety/product-limit explanations. Fixtures remain available only as clearly labeled offline preview/test/demo data. This cleanup is UI/docs/test-only and does not change backend endpoints, API behavior, daemon behavior, storage behavior, or local-only guardrails. Aethra has demo smoke and review-readiness tests plus a manual review checklist for screenshot/reviewer passes. The checklist does not generate screenshots. Aethra stays read-only with no startup auto-load, polling, persistence of live responses, telemetry, cloud calls, prompt submission, route execution, connector mutation, policy mutation, package generation, package validation, upload/download/delete actions, or model/runner controls.
