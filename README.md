...
Aethra can manually load the same `/v1/capabilities` metadata in live-local mode on the Model / Runner Status page. Fixture mode remains the default, failed live-local loads keep fixture capability metadata visible, and the dashboard stays read-only with no polling, telemetry, cloud calls, connector mutation, or model/runner controls.
