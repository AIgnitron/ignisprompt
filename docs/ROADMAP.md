...
- Local policy workbench workflow with synthetic policy scenarios, `ignispromptctl policy-scenarios`, copy-safe policy reports, ignored local-evidence/policy package output, CLI list/validate behavior, `make policy-check`, demo workflow integration, and Aethra read-only policy package preview.
