...
2. Confirm the banner says the dashboard is read-only, manual refresh only, no telemetry, and no cloud calls by default.
...
- Evidence Bundle Viewer: local evidence package index and read-only boundary note.
- Audit Events: audit summary and empty or loaded state.
- Sustainability Preview: proxy metric cards and read-only dashboard boundary.
