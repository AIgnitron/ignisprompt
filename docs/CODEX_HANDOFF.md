...
IgnisPrompt now exposes `GET /v1/operations/summary` for read-only local operations metadata. Aethra can show the summary on Overview and Audit Events after manual refresh, with fixture fallback otherwise. The summary is aggregate local-preview metadata only: daemon status, endpoint availability, audit counts, recent event types, and recent activity counts. It does not expose raw prompts, raw request bodies, full audit event bodies, secrets, telemetry, cloud activity, route execution, model execution, connector mutation, production monitoring status, certification status, or signed evidence.

IgnisPrompt now exposes `GET /v1/routing/policy-summary` for read-only local routing policy metadata. Aethra can show the summary on Overview and Routing Explorer after manual refresh, with fixture fallback otherwise. The summary is descriptive local-preview metadata only: route categories, decision inputs, model selection hints, connector policy hints, audit policy hints, warnings, next steps, and safety boundaries. It does not execute routes, submit prompts, execute models, mutate policy or manifests, mutate connectors or runners, call cloud services, send telemetry, return secrets, expose raw prompts or request bodies, or prove legal accuracy, production readiness, compliance, certification, or attestation.

IgnisPrompt now exposes `GET /v1/evidence/packages` for read-only local evidence package index metadata. Aethra can show the index on Overview and Evidence Bundle Viewer after manual refresh, with fixture fallback otherwise. The index is observational local-preview metadata only: safe relative package paths, package type guesses, cheap file counts and sizes, artifact indicators, warnings, and boundary notes. It does not read full evidence contents, expose raw prompts, expose raw request bodies, expose audit event bodies, upload, download, delete, mutate files, generate packages, validate package correctness, call cloud services, send telemetry, expose secrets, or prove attestation, certification, compliance, legal correctness, or production readiness.
...
Aethra can manually load `GET /v1/models/inventory`, `GET /v1/models/readiness`, `GET /v1/capabilities`, and `GET /v1/status/models` on the Model / Runner Status screen. It can also manually load `GET /v1/routing/policy-summary` on Routing Explorer, `GET /v1/evidence/packages` on Evidence Bundle Viewer, and `GET /v1/metrics/sustainability?period=30d` on the Sustainability Preview screen in live-local mode, with fixture fallback data remaining visible until a successful manual load. The top-level local daemon refresh loads those same read-only metadata surfaces together with `GET /v1/operations/summary`. Live loading remains explicit/manual, and the UI presents these values as local daemon hints, read-only inventory metadata, read-only readiness hints, descriptive routing policy metadata, read-only evidence package metadata, aggregate operations metadata, and methodology-dependent proxy estimates.
