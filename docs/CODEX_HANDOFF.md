...
Aethra can manually load `GET /v1/models/inventory`, `GET /v1/models/readiness`, `GET /v1/capabilities`, and `GET /v1/status/models` on the Model / Runner Status screen. It can also manually load `GET /v1/routing/policy-summary` on Routing Explorer, `GET /v1/evidence/packages` on Evidence Bundle Viewer, and `GET /v1/metrics/sustainability?period=30d` on the Sustainability Preview screen in live-local mode. The top-level local daemon refresh loads those same read-only metadata surfaces together with `GET /v1/operations/summary`. Live loading remains explicit/manual, and failed live surfaces stay failed/unavailable rather than being replaced by fixtures.
