...
- `docs/LOCAL_SLM_RUNNER_CONTROL_DESIGN.md` is the design source for future local SLM runner process status and guarded operator controls. It is docs-only: no runner start/stop endpoints, CLI commands, Aethra controls, process manager, model execution, model downloads, cloud calls, or routing changes are implemented by that design note.
