...
- `v0.1.2-local-preview` was published from #143. It includes #141, #142, and the #143 patch release package. Do not move or recreate `v0.1.1-local-preview`.
...
- `v0.1.7-local-preview` was the previous published local preview release. It includes the local policy workbench arc, `ignispromptctl policy-scenarios`, policy packages, Aethra Local Policy Workbench, `make policy-check`, demo workflow alignment, and conservative package validation hardening.
...
- Audit appends use a shared in-process write lock and persist each JSON object plus newline as one complete JSONL record before adding the event to process memory. Required route-explain and chat-completion operations return sanitized `AUDIT_WRITE_FAILED` errors when persistence fails, do not create memory-only events, and do not cache unaudited completions.
- Feature-gated GGUF runs use private per-run temporary directories with owner-only directory and prompt/stdout/stderr file permissions on Unix. Experimental MCP stdio input is bounded to 1 MiB per line and rejects oversized messages without echoing their contents.
- The HTTP daemon keeps the default loopback bind and local-origin CORS for browser clients. Non-loopback HTTP binds require explicit `--allow-non-loopback-cors` or `IGNISPROMPT_ALLOW_NON_LOOPBACK_CORS=true`, which remains an unsafe local-preview operator decision rather than auth, TLS, production readiness, or security certification. Authentication, authorization, TLS, and stricter production CORS remain required future work and are intentionally not implemented by this hardening change.
...
- `GET /v1/models/readiness` reports read-only local model readiness hints by comparing manifest declarations, local inventory metadata, file extension support, shard filename hints, and runner status hints. It does not execute models, execute routes, download/delete/upload files, mutate manifests, read full model contents, hash large files, expose secrets, call cloud services, or prove executable inference, model quality, production readiness, compliance, or legal accuracy. `ignispromptctl model-readiness` reads the endpoint as either terminal summary or `--json`; the doctor/readiness check treats it as informational.
...
- `GET /v1/runners/status` and `ignispromptctl runners status` provide read-only local runner process status metadata from PR #217. The contract reports conservative runner state, uses daemon-authoritative `actions_allowed`, and includes read-only/no-mutation boundaries. It does not start, stop, restart, or manage runner processes; does not execute models or routes; does not download models; and does not call cloud services.
- PR #218 adds guarded local runner lifecycle request surfaces: `POST /v1/runners/{runner_id}/start`, `POST /v1/runners/{runner_id}/stop`, `ignispromptctl runners start <runner_id> --confirm-local-runner-control`, and `ignispromptctl runners stop <runner_id> --confirm-local-runner-control`. The daemon flag `--enable-runner-lifecycle-controls` / `IGNISPROMPT_ENABLE_RUNNER_LIFECYCLE_CONTROLS` defaults to false. Current built-in runners remain unmanaged and fail closed with `actions_allowed: ["none"]`; confirmed attempts append local audit events, but no process manager, spawn, stop, kill, shell command, model execution, route execution, model download, cloud call, telemetry, polling, or browser persistence is added.
- PR #219 adds an Aethra runner process status and guarded Operator Mode panel on Model / Runner Status. Operator Mode is session-only, off by default, unavailable in fixture mode, requires manually loaded runner process status from the current daemon URL, and requires final per-action confirmation before Aethra sends a lifecycle POST. Aethra revalidates the runner ID, current daemon URL, latest loaded runner row, and latest `actions_allowed` before submitting; changing the daemon URL invalidates old runner action authority. Current daemon v0.1 lifecycle responses are rejected-only; Aethra treats `accepted: true` lifecycle bodies as incompatible contract drift until a future daemon schema adds a valid accepted result. A receipt or unknown transport outcome does not optimistically update displayed process status, turns Operator Mode off, and requires manual runner process status and audit-event refresh before another action. Fixture mode never sends lifecycle POSTs. This adds no polling, browser persistence, startup request, real process manager, model execution, route execution, downloads, cloud calls, or telemetry.
- `GET /v1/operations/summary` reports aggregate local-preview daemon operations metadata for daemon status, endpoint availability, audit counts, recent event types, and recent local activity counts. It does not expose raw prompts, raw request bodies, full audit event bodies, secrets, machine identifiers, absolute local paths, telemetry, cloud activity, route execution, model execution, connector mutation, production monitoring status, certification status, or signed evidence. `ignispromptctl operations-summary` reads the endpoint as either terminal summary or `--json`; the doctor/readiness check treats it as informational.
- `GET /v1/routing/policy-summary` reports descriptive local-preview routing policy metadata for route categories, decision inputs, model selection hints, connector policy hints, audit policy hints, warnings, next steps, and safety boundaries. It does not execute routes, submit prompts, execute models, mutate policy or manifests, mutate connectors or runners, call cloud services, send telemetry, return secrets, expose raw prompts or request bodies, or prove legal accuracy, production readiness, compliance, certification, or attestation. `ignispromptctl routing-policy` reads the endpoint as either terminal summary or `--json`; the doctor/readiness check treats it as informational.
- `GET /v1/evidence/packages` reports read-only local evidence package index metadata from the repo `local-evidence` root. It uses bounded traversal, skips symlinks, returns safe relative paths, package type guesses, cheap file counts and sizes, artifact indicators, warnings, and boundary notes, and handles missing evidence roots as local-preview information. It does not generate packages, validate package correctness, upload, download, delete, mutate files, read full evidence contents, expose raw prompts, expose raw request bodies, expose audit event bodies, expose secrets, call cloud services, send telemetry, or prove attestation, certification, compliance, legal correctness, or production readiness. `ignispromptctl evidence-packages` reads the endpoint as either terminal summary or `--json`; the doctor/readiness check treats it as informational.
...
- `ignispromptctl doctor` checks required local preview endpoints for `/health`, `/v1/status/version`, `/v1/models`, and `/v1/status/models`, plus informational checks for model inventory, model readiness, capabilities, operations summary, routing policy summary, evidence package index, and sustainability metrics. It supports `--json`, exits non-zero when required checks fail, and prints local next steps for common failures. It does not add telemetry, cloud calls, GitHub calls, update checks, external lookup, persistence, uploads, package generation, package validation, model controls, runner controls, or command execution beyond local HTTP reads.
...
- `docs/releases/v0.1.7-local-preview.md` is the v0.1.7 local preview release record for the local policy workbench arc.
- `docs/releases/v0.1.8-local-preview.md` is the v0.1.8 published local preview release record for the Local Demo Studio arc and PR #188 hardening work.
...
- Aethra Overview can manually load daemon version status in live-local mode and otherwise shows not-loaded or failed live status. Offline preview fixture metadata is separate and clearly labeled.
...
- explicitly labeled offline preview fixtures for tests/demo views
- read-only
- no telemetry
...
The live metadata controls use the configured loopback/local daemon base URL. The top-level **Refresh local daemon data** action loads these supported read-only surfaces together and reports attempted, loaded, unavailable, failed, and not-loaded counts. Failed sections remain failed/unavailable until a successful manual refresh instead of silently showing fixture values as product state. These controls do not auto-load on startup, poll, persist state in local storage or session storage, add telemetry, make cloud calls by default, submit prompts, execute routes or models, generate/validate packages, upload/download/delete files, or add connector, model, runner, policy, or manifest controls.

Aethra also includes a fixture-backed evidence bundle viewer for manifest, validation summary, archive metadata preview, safe local-preview CLI snippets, clipboard-only Markdown and JSON report export helpers, and a read-only local evidence package index when manually refreshed from the daemon. It is read-only and local-preview only. It does not extract archives, upload data, persist bundle state, generate packages, validate packages as certified, or read arbitrary local paths.
...
IgnisPrompt now exposes `GET /v1/models/readiness` for read-only local model readiness hints. Aethra can show the readiness summary on the Model / Runner Status page and a small Overview summary after manual refresh. Readiness is advisory metadata from manifests, inventory, file format hints, shard filename hints, and runner status hints only; it does not execute models, route requests, download/delete/upload files, mutate manifests, read full model contents, hash large files, call cloud services, return secrets, or prove executable inference, model quality, readiness, compliance, legal accuracy, certification, or production status.
...
IgnisPrompt now exposes `GET /v1/operations/summary` for read-only local operations metadata. Aethra can show the summary on Overview and Audit Events after manual refresh; otherwise the live surface stays not loaded or failed. The summary is aggregate local-preview metadata only: daemon status, endpoint availability, audit counts, recent event types, and recent activity counts. It does not expose raw prompts, raw request bodies, full audit event bodies, secrets, telemetry, cloud activity, route execution, model execution, connector mutation, production monitoring status, certification status, or signed evidence.

IgnisPrompt now exposes `GET /v1/routing/policy-summary` for read-only local routing policy metadata. Aethra can show the summary on Overview and Routing Explorer after manual refresh; otherwise the live surface stays not loaded or failed. The summary is descriptive local-preview metadata only: route categories, decision inputs, model selection hints, connector policy hints, audit policy hints, warnings, next steps, and safety boundaries. It does not execute routes, submit prompts, execute models, mutate policy or manifests, mutate connectors or runners, call cloud services, send telemetry, return secrets, expose raw prompts or request bodies, or prove legal accuracy, production readiness, compliance, certification, or attestation. Aethra no longer submits live route-explain requests from the dashboard.

IgnisPrompt now exposes `GET /v1/evidence/packages` for read-only local evidence package index metadata. Aethra can show the index on Overview and Evidence Bundle Viewer after manual refresh; otherwise the live surface stays not loaded or failed. The index is observational local-preview metadata only: safe relative package paths, package type guesses, cheap file counts and sizes, artifact indicators, warnings, and boundary notes. It does not read full evidence contents, expose raw prompts, expose raw request bodies, expose audit event bodies, upload, download, delete, mutate files, generate packages, validate package correctness, call cloud services, send telemetry, expose secrets, or prove attestation, certification, compliance, legal correctness, or production readiness.
...
Aethra can manually load `GET /v1/status/version` on the Overview screen in live-local mode. The main dashboard does not substitute fixture release status while waiting for a successful manual load, and unreachable daemon/schema errors keep a clear failed or unavailable state. Aethra does not poll, persist this metadata, add telemetry, call cloud services, call GitHub, or perform release/update checks.
...
Aethra main product pages now keep detailed explanatory/status guidance in Help. Overview keeps the full live-local daemon dashboard, while Model / Runner Status, Routing Explorer, Audit Events, Evidence Bundle Viewer, and Sustainability Preview focus on concise status labels, data cards, tables, actions, collapsed details where needed, and empty states. No model mutation controls are added. Guarded runner lifecycle requests exist only in explicit live-local Operator Mode, require daemon-authorized actions and per-action confirmation, and may be rejected by the daemon. This UI/docs/test cleanup does not add polling, telemetry, storage persistence, cloud calls, GitHub calls, update checks, command execution, backend changes, or API shape changes.

Aethra empty states explain what data is missing, whether the current view is live-local or explicitly labeled offline preview fixture, and what local action to try next, such as starting `./scripts/start-dev.sh`, running `./scripts/smoke.sh`, or manually refreshing the relevant panel. These empty states are local-only UI copy. They do not auto-load data, poll endpoints, persist state, add telemetry, call cloud services, call GitHub, or perform update checks.
...
Aethra can manually load `GET /v1/models/inventory`, `GET /v1/models/readiness`, `GET /v1/capabilities`, `GET /v1/status/models`, and `GET /v1/runners/status` on the Model / Runner Status screen. It can also manually load `GET /v1/routing/policy-summary` on Routing Explorer, `GET /v1/evidence/packages` on Evidence Bundle Viewer, and `GET /v1/metrics/sustainability?period=30d` on the Sustainability Preview screen in live-local mode. The top-level local daemon refresh loads those same metadata surfaces together with `GET /v1/operations/summary`. Live loading remains explicit/manual, and failed live surfaces stay failed/unavailable rather than being replaced by fixtures. Runner lifecycle requests are only available from the Model / Runner Status page after live-local runner process status is loaded from the current daemon URL, Operator Mode is enabled, and the operator confirms a specific daemon-authorized action.

Aethra's visual capability/status matrix uses manually loaded `GET /v1/capabilities` rows in live-local mode and otherwise stays not loaded, failed, unavailable, or empty. Offline preview fixture capability metadata is separate and clearly labeled. This adds no polling, no telemetry, no cloud calls, no connector controls, and no model mutation controls. Guarded runner lifecycle requests are limited to the Model / Runner Status Operator Mode panel.
