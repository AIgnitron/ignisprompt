...
- Latest known main commit: `f103a1c fix: harden local runtime and CI boundaries (#182)`
...
- Post-v0.1.6 development adds a local policy workbench arc with `ignispromptctl policy-scenarios`, synthetic policy scenarios, copy-safe policy reports, local policy packages under `local-evidence/policy/`, `make policy-check`, demo workflow integration, and Aethra Local Policy Workbench. The policy surface is local-preview only, uses synthetic scenarios, and treats route summaries as hints rather than guarantees.
...
- `make policy-check` runs deterministic local policy workbench checks without requiring a live daemon. It verifies policy scenario CLI output, report output, package generation/list/validate behavior, demo workflow command construction, and Aethra policy wording alignment.
...
Aethra includes a Local Policy Workbench that shows fixture-backed synthetic policy scenarios, read-only route hints, policy package preview metadata, and copy-safe policy report snippets. It does not execute commands, upload files, read arbitrary local paths, extract archives, poll endpoints, persist policy data, add telemetry, add cloud calls, add backend endpoints, or add model or runner controls.
