...
- Latest known main commit: `850242c docs: prepare v0.1.5 local preview release (#174)`
...
Aethra includes a Local Operator Console that combines fixture-backed local preview readiness, readiness package status, evidence workflow status, demo next steps, local safety boundaries, and copy-only command recipes. It is read-only and does not execute commands, upload files, poll endpoints, persist operator data, add telemetry, add cloud calls, add backend endpoints, or add model or runner controls.
