...
- `GET /v1/operations/summary`: returns aggregate local-preview daemon operations metadata, including daemon status, endpoint availability, audit counts, recent event types, and recent activity counts. It does not expose raw prompts, raw request bodies, secrets, machine identifiers, absolute paths, telemetry, cloud activity, route execution, model execution, connector mutation, production monitoring status, certification status, or signed evidence.
- `GET /v1/routing/policy-summary`: returns descriptive local-preview routing policy metadata, including route categories, decision inputs, model selection hints, connector policy hints, audit policy hints, safety boundaries, warnings, and next steps. It does not execute routes, submit prompts, execute models, mutate policy or manifests, mutate connectors or runners, call cloud services, send telemetry, return secrets, expose raw prompts, or claim production/legal/compliance readiness.
- `GET /v1/evidence/packages`: returns read-only local evidence package index metadata from the repo local evidence root. It reports safe relative package paths, package type guesses, cheap file counts and sizes, artifact indicators, warnings, and boundary notes without reading full evidence contents, exposing raw prompts, exposing raw request bodies, exposing audit event bodies, uploading, downloading, deleting, mutating files, generating packages, validating package correctness, or making attestation/certification/compliance/legal correctness claims.
...
`GET /v1/operations/summary` is a local-preview foundation for observing aggregate local daemon operations without exposing request content. The daemon derives the summary from in-memory process state and audit event metadata already held by the local daemon: daemon version/start time/uptime, implemented endpoint availability flags, audit event totals, recent event type names, and recent activity/error counts.

The endpoint is read-only and aggregate-only. It does not return raw prompts, raw request bodies, full audit event bodies, secrets, machine identifiers, usernames, absolute local paths, telemetry, cloud provider checks, external network data, production monitoring status, certification status, or signed evidence. It does not execute routes or models, mutate connectors, write files, poll, upload, download, or change daemon configuration.

## Local routing policy summary

`GET /v1/routing/policy-summary` is a local-preview foundation for explaining the daemon's routing policy shape without performing a route request. The daemon reports descriptive route categories, policy mode, decision inputs, model selection hints, connector policy hints, audit policy hints, conservative warnings, and explicit safety boundaries.

The endpoint is read-only and descriptive only. It does not execute routes, submit prompts, execute models, mutate policy, mutate manifests, mutate connectors, mutate runners, call cloud services, send telemetry, inspect secrets, expose raw prompts or request bodies, or prove model quality, legal accuracy, production readiness, compliance status, certification, or attestation.

## Local evidence package index

`GET /v1/evidence/packages` is a local-preview foundation for observing ignored local evidence package outputs without creating, validating, uploading, or certifying them. The daemon scans only the repo `local-evidence` root, skips hidden entries and symlinks, applies conservative package, file, and depth limits, and returns metadata derived from safe relative paths, filenames, cheap file sizes, modified timestamps, and artifact-name indicators.

The endpoint is read-only and metadata-only. It does not scan arbitrary user directories, follow traversal outside the evidence root, expose absolute paths, read full evidence contents, parse raw prompts, expose raw request bodies, expose audit event bodies, upload files, download files, delete files, mutate directories, generate packages, validate package correctness, execute routes, execute models, call cloud services, send telemetry, expose secrets, or prove attestation, certification, compliance, legal correctness, or production readiness.
